...
interface CacheEntry { verdict: 'safe' | 'unsafe'; expiresAt: number }
...
export class SafeBrowsingClient {
  ...
  async check(url: string): Promise<'safe' | 'unsafe'> {
    ...
    if (!res.ok) return 'safe'; // fail-open: don't block on quota errors
    const json = await res.json() as { matches?: unknown[] };
    const verdict: 'safe' | 'unsafe' = (json.matches?.length ?? 0) > 0 ? 'unsafe' : 'safe';
