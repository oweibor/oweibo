...
export type SmeArtifactKind =
  | 'task_output'
  | 'rubric_evaluation'
  | 'compliance_decision'
  | 'regulatory_feed_item';

export type SmeQueueState =
  | 'pending'      // newly sampled, no reviewers assigned
  | 'assigned'     // ≥1 reviewer started
  | 'reviewed'     // required_reviews submitted
  | 'aggregated'   // feedback aggregator processed
  | 'closed';      // terminal (acted on or expired)
 ...
 */
export type SmeSuggestionTargetKind =
  | 'ontology_glossary'
  | 'ontology_entity'
  | 'rubric_criterion'
  | 'compliance_rule'
...
export type SmeAggregatedFeedbackState =
  | 'pending_review'
  | 'approved'
  | 'rejected'
  | 'superseded';
