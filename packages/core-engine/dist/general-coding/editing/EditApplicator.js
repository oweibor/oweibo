...
exports.VfsValidationError = VfsValidationError;
/**
...
class EditApplicator {
    ...
    warmPool;
    vfsValidator;
    constructor(git, warmPool, vfsValidator) {
        this.git = git;
        this.warmPool = warmPool;
        this.vfsValidator = vfsValidator;
    }
    async apply(repoRoot, proposal, taskId, sessionId, secCtx = { permissions: ['workspace:write'] }) {
        const editedFiles = [];
        const normalizedRoot = (0, path_1.normalize)((0, path_1.resolve)(repoRoot));
        // Path traversal guard
        for (const { filePath } of [...proposal.proposal, ...proposal.newFiles]) {
            const resolved = (0, path_1.resolve)((0, path_1.join)(normalizedRoot, filePath));
        ...
        try {
            for (const { filePath, diff } of proposal.proposal) {
                const hostPath = (0, path_1.join)(normalizedRoot, filePath);
                const fileContent = await (0, promises_1.readFile)(hostPath, 'utf8').catch(() => '');
                await sandbox.execute(`mkdir -p /tmp/dryrun && printf '%s' ${JSON.stringify(fileContent)} > /tmp/dryrun/target`, 'bash');
                const result = await sandbox.execute(`cd /tmp/dryrun && patch --dry-run -p0 target <<'PATCH'\n${diff}\nPATCH`, 'bash');
        ...
        for (const { filePath, diff } of proposal.proposal) {
            const hostPath = (0, path_1.join)(normalizedRoot, filePath);
            const original = await (0, promises_1.readFile)(hostPath, 'utf8').catch(() => '');
        ...
        try {
            ...
            for (const { filePath, diff } of proposal.proposal) {
                const hostPath = (0, path_1.join)(normalizedRoot, filePath);
                const original = await (0, promises_1.readFile)(hostPath, 'utf8');
                const patched = this.applyUnifiedDiff(original, diff);
                await (0, promises_1.writeFile)(hostPath, patched, 'utf8');
                editedFiles.push(filePath);
            }
            // Write new files
            for (const { filePath, content } of proposal.newFiles) {
                const hostPath = (0, path_1.join)(normalizedRoot, filePath);
                await (0, promises_1.mkdir)((0, path_1.dirname)(hostPath), { recursive: true });
                await (0, promises_1.writeFile)(hostPath, content, 'utf8');
                editedFiles.push(filePath);
            }
            ...
            const commitHash = await this.git.commit(normalizedRoot, `oweibo[${taskId.slice(0, 8)}]: ${proposal.explanation.slice(0, 72)}`);
            if (stashCreated)
     ...
     */
    applyUnifiedDiff(original, diff) {
...
exports.EditApplicator = EditApplicator;
//# sourceMappingURL=EditApplicator.js.map