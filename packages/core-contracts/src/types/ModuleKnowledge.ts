...
export interface UserFlowDoc {
  readonly name: string;             // e.g. "Place an order"
  readonly actor: string;            // e.g. "Customer"
  readonly steps: readonly string[]; // plain-English steps, no code references
  readonly outcome: string;          // e.g. "Order is confirmed and payment is captured"
...
export interface GlossaryEntry {
  ...
  readonly definition: string;  // one sentence, non-technical
}

/** ExampleUsageDoc (v8) — human-readable usage example lifted from testFiles by ExecutorAgent. */
export interface ExampleUsageDoc {
  readonly title: string;        // e.g. "Creating a new product"
  readonly description: string;
  readonly codeSnippet: string;  // relevant test/usage code, lightly cleaned
  readonly language: string;     // e.g. "typescript", "bash"
}
