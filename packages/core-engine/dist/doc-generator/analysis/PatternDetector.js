...
class PatternDetector {
    ...
    // ── Structural (FS-based) ───────────────────────────────────────────────────
    detectMonorepo(paths) {
        const hasWorkspace = paths.some((p) => p.endsWith('pnpm-workspace.yaml') || p.endsWith('lerna.json') || p.endsWith('nx.json'));
        return {
            kind: 'monorepo',
    ...
    detectFactory(files) {
        const evidence = [];
        for (const f of files) {
            const hasFactory = f.exports.some((s) => /^(create|build|make|produce)[A-Z]/.test(s.name) && s.kind === 'function');
            if (hasFactory)
                evidence.push(f.filePath);
        }
        return {
    ...
    detectPipeline(files) {
        const evidence = [];
        for (const f of files) {
            const hasMiddleware = f.exports.some((s) => s.kind === 'function' && /[Mm]iddleware|[Hh]andler|[Ii]nterceptor/.test(s.name));
            if (hasMiddleware)
                evidence.push(f.filePath);
        }
        return {
    ...
    detectSingleton(files) {
        ...
        for (const f of files) {
            const hasSingleton = f.exports.some((s) => {
                ...
                return members.some((m) => m.name === 'getInstance' && m.visibility === 'public') && members.some((m) => m.visibility === 'private' && m.name === 'instance');
            });
            if (hasSingleton)
                evidence.push(f.filePath);
        }
    ...
    detectDI(files) {
        const evidence = [];
        for (const f of files) {
            const hasInject = f.exports.some((s) => s.kind === 'class' && (s.decorators?.some((d) => /Inject|Injectable|Component|Service/i.test(d)) ?? false));
            if (hasInject)
                evidence.push(f.filePath);
        }
        return {
    ...
    detectStrategy(files) {
        const interfaces = files.flatMap((f) => f.exports.filter((s) => s.kind === 'interface'));
        const evidence = [];
        for (const iface of interfaces) {
            const implementations = files.filter((f) => f.exports.some((s) => s.kind === 'class' && s.members?.some((m) => m.name === iface.name)) || f.dependencies.some((d) => d === iface.filePath));
