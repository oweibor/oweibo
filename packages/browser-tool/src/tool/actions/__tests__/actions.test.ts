...
import type { BrowserAction, IBrowserExecutionContext, ISecurityContext } from '@oweibo/core-contracts';

import { AutofillCredentialsAction } from '../AutofillCredentialsAction.js';
import { ExtensionHitlRespondAction } from '../ExtensionHitlRespondAction.js';
import { ImportCookiesAction } from '../ImportCookiesAction.js';
...
function ctx(over: Partial<ISecurityContext> = {}): IBrowserExecutionContext {
  return {
    tenantId: 't1',
    sessionId: 's1',
    taskId: 'task1',
...
describe('ImportCookiesAction', () => {
  test('refuses without allowCookieImport', async () => {
    ...
    const action = new ImportCookiesAction(bridge);
    await expect(
      action.execute({ type: 'import-cookies', domain: 'x.com' }, ctx({ allowCookieImport: false })),
    ).rejects.toThrow(/allowCookieImport/);
  });
  ...
  test('happy path applies cookies and emits event without leaking values', async () => {
    ...
    expect((c.eventEmitter.emit as jest.Mock)).toHaveBeenCalledWith(
      'browser-cookies-imported',
      expect.objectContaining({ domain: 'x.com', cookieCount: 1 }),
    );
  });
});
...
describe('AutofillCredentialsAction', () => {
  test('refuses without allowAutofill', async () => {
    ...
    const action = new AutofillCredentialsAction(bridge);
    await expect(
      action.execute({ type: 'autofill-credentials' }, ctx({ allowAutofill: false })),
    ).rejects.toThrow(/allowAutofill/);
  });
...
describe('ExtensionHitlRespondAction', () => {
  test('rejects direct invocation without internal marker', async () => {
    ...
    const action = new ExtensionHitlRespondAction(resolver);
    const r = await action.execute(
      { type: 'extension-hitl-respond', gateId: 'g1', accept: true } as Extract<BrowserAction, { type: 'extension-hitl-respond' }>,
      ctx(),
    );
    expect(r.success).toBe(false);
  ...
  test('marked invocation resolves the gate', async () => {
    ...
    const action = new ExtensionHitlRespondAction(resolver);
    const a = ExtensionHitlRespondAction.markInternal({
      type: 'extension-hitl-respond' as const, gateId: 'g1', accept: true,
    });
  ...
  test('idempotent when resolver returns false', async () => {
    ...
    const action = new ExtensionHitlRespondAction(resolver);
    const a = ExtensionHitlRespondAction.markInternal({
      type: 'extension-hitl-respond' as const, gateId: 'g1', accept: false,
    });
    const r = await action.execute(a, ctx());
    expect(r.success).toBe(true);
    expect((r.data as { idempotent: boolean }).idempotent).toBe(true);
  });
});
