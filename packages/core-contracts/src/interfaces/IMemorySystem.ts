...
export type MemoryKind =
  | 'user-preference'        // "user prefers Tailwind over Bootstrap"
  | 'architectural-decision' // "chose Postgres over MongoDB because of joins"
  | 'decision-rationale'     // why something was done a particular way
  | 'failure-lesson'         // "last time X failed because Y"
  | 'success-pattern'        // a successful plan/strategy template
  | 'code-landmark'          // "the auth middleware lives in X.ts"
  | 'domain-fact'            // tenant-specific domain knowledge
  | 'conversation-summary'   // rolling summary of a session's chat turns
  | 'project-invariant'      // constraint that holds for the whole project
  | 'open-question'          // unresolved question flagged during a task
  | 'tool-heuristic';        // "for this kind of task, prefer tool X"
...
export interface TaskOutcome {
  ...
  readonly decisions?:  readonly { decision: string; rationale: string }[];
  readonly failures?:   readonly { attempt: string; reason: string }[];
  readonly filesTouched?: readonly string[];
  readonly preferences?:  readonly { key: string; value: string }[];
  readonly invariants?:   readonly { key: string; value: string }[];
