...
exports.toModuleKnowledge = toModuleKnowledge;
function toModuleKnowledge(knowledge, targetModule) {
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function readVersionFromPackageJson(knowledge, rootPath) {
    const dep = knowledge.externalDependencies.find((d) => d.name === 'self' || d.name === rootPath);
    return dep?.version ?? '1.0.0';
}
function mapToModuleEntityDocs(symbols) {
    return symbols
...
function mapCallGraphToEndpoints(callGraph, targetModule) {
    // Infer endpoints from direct calls whose callee name looks like an HTTP handler
    return callGraph
        .filter((e) => e.callerFile.startsWith(targetModule.rootPath) &&
        /^(get|post|put|delete|patch)[A-Z]/.test(e.calleeSymbol))
        .map((e) => {
        const method = e.calleeSymbol.slice(0, e.calleeSymbol.search(/[A-Z]/)).toUpperCase();
        return {
            method,
            path: `/${e.calleeSymbol}`,
...
function filterEvents(callGraph, targetModule, kind) {
    return callGraph
        .filter((e) => e.callType === kind &&
...
function mineInvariantsFromComments(symbols) {
    const results = [];
    for (const s of symbols) {
        if (!s.rawDocumentation)
            continue;
...
function detectExtensionPoints(symbols) {
    return symbols
...
function mineGlossaryFromDocs(symbols, projectSummary) {
    const seen = new Set();
    const entries = [];
    for (const s of symbols) {
        if (!s.rawDocumentation || seen.has(s.name))
            continue;
...
//# sourceMappingURL=ModuleKnowledgeAdapter.js.map