...
import type { ExtendedToolDefinition } from '../ToolRegistry.js';

export const kiloPipelineTool: ExtendedToolDefinition = {
  ...
  handler: async (input) => {
    ...
    const { instruction, scaffoldInput, workspacePath, trustMode, tokenBudget } = input as Record<string, unknown>;
    ...
    if (!res.ok) throw new Error(`kilo pipeline returned ${res.status}: ${await res.text()}`);
    return res.json();
  },
};
