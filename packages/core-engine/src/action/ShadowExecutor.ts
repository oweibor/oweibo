...
export class ShadowExecutor {
   ...
   */
  async recordOutcome(principal: GatePrincipal, outcome: ShadowOutcome): Promise<void> {
    ...
    try {
      ...
      await client.query(
        `UPDATE oweibo.action_proposals
          ...
          WHERE id = $1`,
        [outcome.proposalId, principal.sub, decisionReason, payload],
      );
      ...
      const rejectionDelta =
        !outcome.success || outcome.parity === 'drift' ? 1 : 0;
          ...
          `INSERT INTO oweibo.tenant_action_class_state (
           ...
           ON CONFLICT (tenant_id, action_class) DO UPDATE
             SET observations = oweibo.tenant_action_class_state.observations + 1,
                 ...
                 last_updated = NOW()`,
          [tenant_id, action_class, successDelta, rejectionDelta],
        );
      }
