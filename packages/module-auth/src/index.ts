...
function buildAuthTest(provider: string): string {
...
export class AuthGenerator implements IModuleGenerator {
    ...
    return {
      ...
      knowledgeArtifact: {
        ...
        endpoints: provider !== 'zitadel-native'
          ? [{ method: 'POST' as const, path: '/api/auth/sign-in', filePath: 'src/auth/routes.ts' },
             { method: 'POST' as const, path: '/api/auth/sign-out', filePath: 'src/auth/routes.ts' },
             { method: 'GET' as const, path: '/api/auth/session', filePath: 'src/auth/routes.ts' }]
          : [],
        emittedEvents: [],
        consumedEvents: [],
  ...
  validate(bundle: ArtifactBundle): ValidationResult {
    ...
    if (bundle.files.length === 0) errors.push({ code: 'EMPTY_BUNDLE', message: 'Auth module produced no files' });
