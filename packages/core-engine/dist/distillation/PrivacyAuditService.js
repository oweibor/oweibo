...
exports.PrivacyAuditService = void 0;
class PrivacyAuditService {
    ...
    async overview() {
        const [lessons, buckets, releasable, rejects7d, tenants] = await Promise.all([
            this.pool.query(`SELECT COUNT(*) AS cnt FROM oweibo.platform_lessons`),
            this.pool.query(`SELECT COUNT(DISTINCT bucket_key) AS cnt FROM oweibo.platform_lessons`),
            this.pool.query(`SELECT COUNT(*) AS cnt FROM oweibo.releasable_buckets`),
            this.pool.query(`SELECT COUNT(*) AS cnt FROM oweibo.dlp_rejects WHERE rejected_at > NOW() - INTERVAL '7 days'`).catch(() => ({ rows: [{ cnt: '0' }] })),
            this.pool.query(`SELECT COUNT(DISTINCT tenant_hash) AS cnt FROM oweibo.platform_lesson_tenants`),
        ]);
    ...
    async dlpRejectsTimeline(days = 14) {
       ...
       ORDER BY day, stage`, [days]).catch(() => ({ rows: [] }));
    ...
    async dlpRejectsByStage(days = 14) {
        const res = await this.pool.query(`SELECT reject_stage AS stage, COUNT(*)::text AS cnt
       FROM oweibo.dlp_rejects
       WHERE rejected_at > NOW() - ($1::int * INTERVAL '1 day')
       GROUP BY reject_stage
       ORDER BY COUNT(*) DESC`, [days]).catch(() => ({ rows: [] }));
        return res.rows.map(r => ({
            stage: r.stage,
            count: parseInt(r.cnt, 10),
        }));
     ...
     */
    async blockedBuckets(limit = 50) {
        const res = await this.pool.query(`SELECT pl.bucket_key,
              pl.role,
              pl.slot_id,
...
exports.PrivacyAuditService = PrivacyAuditService;
//# sourceMappingURL=PrivacyAuditService.js.map