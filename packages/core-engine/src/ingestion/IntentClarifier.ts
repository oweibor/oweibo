...
export interface ClassifiedIntent {
  ...
  readonly repoPath?: string;
  readonly clarified: string;   // refined instruction after any clarification dialogue
}
...
export class IntentClarifier {
   ...
   */
  async classifyTaskMode(intent: RawIntent): Promise<ClassifiedIntent> {
    const systemPrompt = [
    ...
    let taskMode: 'factory' | 'general-coding' = 'factory';
    let repoPath: string | undefined;

    try {
      const response = await this.llm.generate({ systemPrompt, userPrompt, responseFormat: 'json' });
      const parsed   = JSON.parse(response.output) as { taskMode: string; repoPath: string | null };
  ...
  buildTask(
    ...
    if (classified.taskMode === 'general-coding' && !classified.repoPath) {
      throw new Error(
        '[IntentClarifier] general-coding tasks require a repoPath. ' +
