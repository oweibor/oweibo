...
export class LanguageAnalyzerRegistry {
  private readonly byLanguage = new Map<CodeLanguage, ILanguageAnalyzer>();
  private fallback: ILanguageAnalyzer | null = null;
  ...
  async analyzeDirectory(
    ...
    signal?:   AbortSignal,
  ): Promise<{ analyses: Awaited<ReturnType<ILanguageAnalyzer['analyzeDirectory']>>; skipped: string[] }> {
    // Group files by language so each analyzer gets a single analyzeDirectory() call
    const groups = new Map<ILanguageAnalyzer, string[]>();
    ...
    const allAnalyses: ReturnType<ILanguageAnalyzer['analyzeDirectory']> extends Promise<infer A> ? A : never[] = [];
    for (const [analyzer, paths] of groups) {
      signal?.throwIfAborted();
      const results = await analyzer.analyzeDirectory(rootPath, paths, signal);
      (allAnalyses as typeof results[number][]).push(...results);
    }

    return { analyses: allAnalyses as Awaited<ReturnType<ILanguageAnalyzer['analyzeDirectory']>>, skipped };
  }
}
