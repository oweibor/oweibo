...
exports.OperationalModeService = exports.OperationDisabledError = exports.MODE_NAMES = void 0;
...
class OperationDisabledError extends Error {
    operation;
    currentMode;
    requiredMode;
    constructor(operation, currentMode, requiredMode) {
        super(`Operation '${operation}' is disabled at platform mode ${currentMode} (${exports.MODE_NAMES[currentMode]}). ` +
...
exports.OperationDisabledError = OperationDisabledError;
...
class OperationalModeService {
    ...
    cachedMode = null;
    cacheExpiresAt = 0;
    TTL_MS = 30_000; // 30-second cache per §17.5.1
    constructor(pool, 
    ...
    async getMode() {
        ...
        try {
            const result = await this.pool.query(`SELECT current_mode FROM oweibo.platform_operational_mode WHERE singleton_id = TRUE`);
            const raw = result.rows[0]?.current_mode ?? 5;
            const mode = Math.max(0, Math.min(5, raw));
    ...
    async getModeState() {
        try {
            const result = await this.pool.query(`SELECT current_mode, set_by, set_at, reason, auto_trigger
          FROM oweibo.platform_operational_mode WHERE singleton_id = TRUE`);
            const row = result.rows[0];
            if (!row)
                return null;
            return {
                currentMode: Math.max(0, Math.min(5, row.current_mode)),
                setBy: row.set_by,
                setAt: row.set_at,
                reason: row.reason,
     ...
     */
    async assertAllowed(op) {
        const mode = await this.getMode();
        const required = MIN_MODE_FOR[op];
        if (mode < required) {
     ...
     */
    async setMode(newMode, opts) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            // Read current mode inside the transaction
            const current = await client.query(`SELECT current_mode FROM oweibo.platform_operational_mode WHERE singleton_id = TRUE FOR UPDATE`);
            ...
            await client.query(`UPDATE oweibo.platform_operational_mode
         SET current_mode = $1, set_by = $2, set_at = NOW(), reason = $3, auto_trigger = $4
         WHERE singleton_id = TRUE`, [newMode, opts.setBy, opts.reason, opts.autoTrigger ?? null]);
            // Write audit log
            await client.query(`INSERT INTO oweibo.operational_mode_transitions
           (from_mode, to_mode, set_by, reason, auto_trigger)
         VALUES ($1, $2, $3, $4, $5)`, [fromMode, newMode, opts.setBy, opts.reason, opts.autoTrigger ?? null]);
            ...
            console.log(`[OperationalModeService] Mode transition: ${fromMode} (${exports.MODE_NAMES[fromMode]}) ` +
                `→ ${newMode} (${exports.MODE_NAMES[newMode]}) by=${opts.setBy} reason="${opts.reason}"`);
        }
        catch (err) {
            await client.query('ROLLBACK');
            throw err;
     ...
     */
    async getTransitionHistory(limit = 50) {
        try {
            const result = await this.pool.query(`SELECT id, from_mode, to_mode, set_by, set_at, reason, auto_trigger
         FROM oweibo.operational_mode_transitions
         ORDER BY set_at DESC LIMIT $1`, [limit]);
            return result.rows.map(r => ({
                id: r.id,
                fromMode: Math.max(0, Math.min(5, r.from_mode)),
                toMode: Math.max(0, Math.min(5, r.to_mode)),
                setBy: r.set_by,
                setAt: r.set_at,
                reason: r.reason,
...
exports.OperationalModeService = OperationalModeService;
const INVALIDATION_CHANNEL = 'platform:mode:invalidate';
//# sourceMappingURL=OperationalModeService.js.map