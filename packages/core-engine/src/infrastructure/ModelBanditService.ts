...
export interface ModelArm {
  ...
  readonly modelId: string;   // opaque model identifier (e.g. 'claude-haiku', 'qwen2.5:7b')
  alpha: number;
  beta:  number;
}
...
const DEFAULT_TIER_MAP: Record<string, ModelTier> = {
  file_read:     'small',
  symbol_lookup: 'small',
  governance:    'small',
  summarisation: 'small',
...
export class ModelBanditService {
   ...
   */
  async draw(taskId: string, category: string): Promise<ModelDrawResult> {
    try {
      ...
      const seed = [...`${taskId}:${category}`].reduce(
        (acc: number, c: string) => (Math.imul(acc, 31) + c.charCodeAt(0)) | 0, 5381,
      ) >>> 0;
  ...
  async recordReward(params: {
    ...
    tier:     ModelTier;
    modelId:  string;
    reward:   number;   // 1.0 = success, 0.0 = failure, 0.5 = no signal
  }): Promise<void> {
    ...
    try {
      ...
      const existing = await client.query(
        `SELECT 1 FROM oweibo.model_bandit_events
         WHERE task_id = $1 AND category = $2 AND model_id = $3`,
        [params.taskId, params.category, params.modelId],
      );
      if ((existing.rowCount ?? 0) > 0) { await client.query('ROLLBACK'); return; }

      await client.query(
        `INSERT INTO oweibo.model_bandit_events (task_id, category, tier, model_id, reward)
         VALUES ($1,$2,$3,$4,$5)`,
        [params.taskId, params.category, params.tier, params.modelId, params.reward],
      );

      if (params.reward >= 0.5) {
        await client.query(
          `UPDATE oweibo.model_bandit_arms
           SET alpha = alpha + $1, updated_at = NOW()
           WHERE category = $2 AND model_id = $3`,
          [params.reward, params.category, params.modelId],
        );
      } else {
        await client.query(
          `UPDATE oweibo.model_bandit_arms
           ...
           WHERE category = $2 AND model_id = $3`,
          [1 - params.reward, params.category, params.modelId],
        );
      }
  ...
  private async loadArms(category: string): Promise<ModelArm[]> {
    const result = await this.pool.query<ModelArm>(
      `SELECT tier, model_id AS "modelId", alpha, beta
       FROM oweibo.model_bandit_arms
       WHERE category = $1 AND active = true
       ORDER BY tier, model_id`,
      [category],
    );
    return result.rows;
  }
