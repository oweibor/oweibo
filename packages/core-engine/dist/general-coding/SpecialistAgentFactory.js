...
exports.SpecialistAgent = exports.SpecialistAgentFactory = exports.RoleWriteBoundaryError = void 0;
...
class RoleWriteBoundaryError extends Error {
    constructor(role, filePath) {
...
exports.RoleWriteBoundaryError = RoleWriteBoundaryError;
// ─────────────────────────────────────────────────────────────────────────────
/**
...
class SpecialistAgentFactory {
    ...
    budgetCache = new Map();
    static BUDGET_CACHE_TTL_MS = 60_000;
    constructor(llm, memory, vault, langfuse, applicator, verifier, redis, tenantRulesLoader) {
        this.llm = llm;
        this.memory = memory;
        this.vault = vault;
        ...
        this.redis = redis;
        this.tenantRulesLoader = tenantRulesLoader;
    }
    /**
     ...
     */
    async loadTenantRulesForClassifier(tenantId) {
     ...
     */
    async spawn(role, task, nodeId, // Gap 5: needed for idempotency key
    secCtx, trace, isRestart = false) {
        ...
        if (!isRestart) {
            ...
            if (!alreadyCounted) {
                ...
                if (current > budget.maxConcurrentSpawns) {
                    ...
                    throw new Error(`[SpecialistAgentFactory] Spawn budget exceeded for task ${task.id}: ` +
        ...
        if (!promptName)
            throw new Error(`[SpecialistAgentFactory] No Langfuse prompt registered for role '${role}'`);
        const promptObj = await this.langfuse.getPrompt(promptName, undefined, { label: 'production' });
        const systemPrompt = promptObj.prompt;
        const agentId = `${role}:${(0, crypto_1.randomUUID)().slice(0, 8)}`;
     ...
     */
    async execute(agent, task, plan, repoMapText, projectRules, skillsPrefix, collectionName, secCtx, trace, sessionId, nodeId) {
        const counterKey = `gc-spawn-active:${task.id}`;
        try {
            ...
            let accumulated = '';
            const proposal = await agent.proposeEdit(plan.instruction, fileContents, repoMapText, (chunk, _fileHint) => { accumulated += chunk; });
            ...
            this.assertWriteBoundary(agent.role, proposal);
     ...
     */
    assertWriteBoundary(role, proposal) {
        const boundaries = ROLE_WRITE_BOUNDARIES[role];
        if (!boundaries)
            return; // no boundary defined for this role — allow all (general-coder path)
        const allPaths = [
        ...
        for (const filePath of allPaths) {
            for (const forbiddenPattern of boundaries.forbidden) {
                if ((0, minimatch_1.minimatch)(filePath, forbiddenPattern, { matchBase: true })) {
    ...
    async loadBudget(tenantId) {
        ...
        try {
            ...
            if (redisVal) {
                const budget = JSON.parse(redisVal);
...
exports.SpecialistAgentFactory = SpecialistAgentFactory;
/**
 ...
 */
class SpecialistAgent extends BaseAgent_js_1.BaseAgent {
    specialistSystemPrompt;
    // Gap 7: TypeScript override — shadows BaseAgent's auto-generated values
    agentId;
    memoryScope;
    constructor(role, agentId, memoryScope, specialistSystemPrompt, llm, memory, trace, taskId, tenantId) {
        super(role, llm, memory, specialistSystemPrompt, trace, taskId, tenantId);
        this.specialistSystemPrompt = specialistSystemPrompt;
        // Assign AFTER super() — these are the correct, role-scoped values
        this.agentId = agentId;
        this.memoryScope = memoryScope;
    }
    async proposeEdit(instruction, fileContents, repoMapContext, onChunk) {
        const userPrompt = `
Repo context:
${repoMapContext}
...
//# sourceMappingURL=SpecialistAgentFactory.js.map