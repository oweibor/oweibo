...
export type QuorumStatus =
  | { kind: 'pending'; approves: number; rejects: number; quorum: number }
  | { kind: 'approved'; approves: number; rejects: number; quorum: number }
  | { kind: 'rejected'; approves: number; rejects: number; quorum: number; reason: 'dissent_veto' | 'quorum_impossible' };

// ── Grant check (used by ActionTrustLadder) ──────────────────────────────
...
export type GrantCheckResult =
  | { kind: 'no_grant' }
  | { kind: 'grant_consumed'; grantId: string };
