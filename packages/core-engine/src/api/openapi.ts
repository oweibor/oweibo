...
export const openapiSpec = {
  ...
  paths: {
    ...
    '/tenants/{tenantId}/connectors/custom': {
      get:  { summary: "List the tenant's registered custom connector manifests", operationId: 'listCustomConnectors', tags: ['Connectors'], parameters: [tenantIdParam()], responses: { '200': { description: 'OK' }, '503': { description: 'Custom connectors not configured' } } },
      post: { summary: 'Register a custom connector manifest (custom.* id, experimental tier)', operationId: 'registerCustomConnector', tags: ['Connectors'], parameters: [tenantIdParam()], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['connectorId', 'displayName', 'category', 'description', 'catalogVersion', 'credentialSchema'], properties: { connectorId: { type: 'string', description: "Must match custom.[a-z0-9][a-z0-9_-]*" }, displayName: { type: 'string' }, category: { type: 'string', enum: ['communication', 'source_control', 'database', 'storage', 'observability', 'payment', 'identity', 'custom'] }, description: { type: 'string' }, catalogVersion: { type: 'string' }, credentialSchema: { type: 'object' }, capabilities: { type: 'array' }, mcpServerUrl: { type: 'string' }, declaredTools: { type: 'array', items: { type: 'string' } } } } } } }, responses: { '201': { description: 'Registered' }, '400': { description: 'Manifest violations (invalid_custom_manifest)' }, '409': { description: 'Already registered' }, '503': { description: 'Custom connectors not configured' } } },
    },
    '/tenants/{tenantId}/connectors/custom/{connectorId}': {
      get:    { summary: 'Custom connector manifest detail', operationId: 'getCustomConnector', tags: ['Connectors'], parameters: [tenantIdParam(), connectorIdParam()], responses: { '200': { description: 'OK' }, '404': { description: 'Not found' }, '503': { description: 'Custom connectors not configured' } } },
      delete: { summary: 'Soft-disable a custom connector (refuses new installs)', operationId: 'disableCustomConnector', tags: ['Connectors'], parameters: [tenantIdParam(), connectorIdParam()], responses: { '204': { description: 'Disabled' }, '404': { description: 'Not found or already disabled' }, '503': { description: 'Custom connectors not configured' } } },
    },
