 ...
 *   xl:  > 256 KB
 */
export function bucketPayloadSize(payloadBytes: number): PayloadSizeBucket {
  if (payloadBytes < 512) return 'xs';
  if (payloadBytes < 4 * 1024) return 'sm';
  if (payloadBytes < 32 * 1024) return 'md';
  if (payloadBytes < 256 * 1024) return 'lg';
  return 'xl';
}
