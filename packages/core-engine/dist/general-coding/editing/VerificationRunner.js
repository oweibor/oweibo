...
class VerificationRunner {
    warmPool;
    codeIntel;
    constructor(warmPool, codeIntel) {
        this.warmPool = warmPool;
        this.codeIntel = codeIntel;
    }
    async run(repoRoot, editedFiles, secCtx) {
        ...
        try {
            ...
            if (affectedTests.length > 0) {
                ...
                const jest = await sandbox.execute(`cd ${repoRoot} && npx jest ${testPattern} --no-coverage --passWithNoTests 2>&1`, 'bash');
                if (jest.exitCode !== 0) {
     ...
     */
    findAffectedTests(repoRoot, editedFiles) {
