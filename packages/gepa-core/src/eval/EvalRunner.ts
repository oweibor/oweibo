...
export interface EvalTask {
  ...
  readonly goldenOutput?: string;   // expected output for deterministic checker
...
export interface EvalScore {
  ...
  readonly qualityPass:    boolean;   // deterministic checker
  readonly qualityScore:   number;    // 0-1 LLM-judge score
...
function deterministicCheck(task: EvalTask, output: string): boolean {
...
export async function runEvalSuite(
  ...
  const scores = await Promise.all(
    tasks.map(t => runEvalTask(t, systemPrompt, llm, promptHash)),
  );
