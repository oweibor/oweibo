...
exports.applyDLPFilter = applyDLPFilter;
...
function applyDLPFilter(text) {
    ...
    for (const { name, re } of PATTERNS) {
        if (re.test(text))
            rejections.push(`pattern:${name}`);
    }
    const lower = text.toLowerCase();
    for (const term of DENYLIST_TERMS) {
        if (lower.includes(term))
            rejections.push(`denylist:${term}`);
    }
    if (hasHighEntropyToken(text))
        rejections.push('entropy:high_entropy_token');
...
//# sourceMappingURL=LessonDLPFilter.js.map