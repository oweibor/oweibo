...
import type {
  ...
  MemoryTier as LegacyMemoryTier,
  MemoryType as LegacyMemoryType,
  RecallResult as LegacyRecallResult,
...
const LEGACY_TYPE_TO_KIND: Record<LegacyMemoryType, MemoryKind> = {
 ...
 */
const KIND_TO_LEGACY: Record<MemoryKind, { type: LegacyMemoryType; tier: LegacyMemoryTier }> = {
 ...
 */
export class LegacyMemorySystemAdapter implements IMemorySystem {
  private readonly orchestrator: IMemoryOrchestrator;
  private readonly endSessionHook?: (tenantId: TenantId, sessionId: SessionId) => Promise<void>;
   ...
   */
  async store(entry: Parameters<IMemorySystem['store']>[0]): Promise<string> {
    ...
    const baseDetail = entry.detail && typeof entry.detail === 'object' && !Array.isArray(entry.detail)
      ...
      : {};
    const detail =
      entry.relevanceTags && entry.relevanceTags.length > 0
   ...
   */
  async recall(params: Parameters<IMemorySystem['recall']>[0]): Promise<LegacyRecallResult[]> {
    ...
    const kinds: MemoryKind[] | undefined = types?.length
   ...
   */
  async warmForTask(
    params: Parameters<IMemorySystem['warmForTask']>[0],
  ): Promise<WarmResult[]> {
   ...
   */
  async endSession(tenantId: TenantId, sessionId: SessionId): Promise<void> {
    if (this.endSessionHook) {
      ...
      this.logger.debug?.(
        '[LegacyMemorySystemAdapter] endSession: no hook supplied; orchestrator STM TTL handles cleanup',
        { tenantId, sessionId },
      );
    }
  }
   ...
   */
  async reinforceMemory(memoryId: string, tenantId: TenantId): Promise<void> {
    this.logger.warn(
      '[LegacyMemorySystemAdapter] reinforceMemory has no equivalent on IMemoryOrchestrator; this call is a no-op',
      { memoryId, tenantId },
    );
  }

  async penaliseMemory(memoryId: string, tenantId: TenantId): Promise<void> {
    this.logger.warn(
      '[LegacyMemorySystemAdapter] penaliseMemory has no equivalent on IMemoryOrchestrator; this call is a no-op',
      { memoryId, tenantId },
    );
  }
  ...
  private rankedToLegacyRecallResult(rm: RankedMemoryEntry): LegacyRecallResult {
    return {
      entry: this.rankedToLegacyEntry(rm),
      score: rm.score,
    };
   ...
   */
  private rankedToLegacyEntry(rm: RankedMemoryEntry): LegacyMemoryEntry {
 ...
 */
function legacyScopeOf(scope: MemoryScope): string {
