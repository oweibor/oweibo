...
exports.makeBrowserCommand = makeBrowserCommand;
/**
...
async function submitBrowserAction(action, opts) {
    ...
    try {
        ...
        if (opts.wait) {
            ...
            await (0, sse_js_1.streamEvents)(task.taskId, (ev) => {
                (0, sse_js_1.printEvent)(ev, opts.json ?? false);
        ...
        console.error('Failed to submit browser action:', err.message);
        process.exit(1);
    }
}
// ── Command builder ───────────────────────────────────────────────────────────
function makeBrowserCommand() {
    const browser = new commander_1.Command('browser')
    ...
    browser
        ...
        await submitBrowserAction({ type: 'navigate', url, backend: opts.backend ?? 'auto' }, opts);
    });
    // ── import-cookies ──────────────────────────────────────────────────────────
    browser
    ...
    browser
        ...
        const tenantId = opts.tenant ?? process.env['OWEIBO_TENANT_ID'] ?? 'default';
        try {
            const result = await client_js_1.api.post('/browser/pair', { tenantId });
            if (opts.json) {
            ...
            if (result.deepLink) {
                ...
                exec(`${open} "${result.deepLink}"`, (err) => {
                    if (err)
                        ...
                        console.log('Extension pairing page opened. Click Connect in the browser.');
                });
            }
            else {
            ...
            console.error('Failed to initiate pairing:', err.message);
            process.exit(1);
        }
    });
    return browser;
