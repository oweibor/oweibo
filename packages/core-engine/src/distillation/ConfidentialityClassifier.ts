...
function scoreOperationalSpecificity(text: string): number {
  let hits = 0;
  for (const re of OPS_MARKERS) if (re.test(text)) hits++;
...
function scoreDomainSpecificity(text: string): number {
  let hits = 0;
  for (const re of DOMAIN_TERMS) if (re.test(text)) hits++;
  return Math.min(hits / 3, 1.0);
}

/** Process fingerprint: reveals a proprietary multi-step process. */
function scoreProcessFingerprint(text: string): number {
...
function scoreVocabularyRarity(text: string): number {
...
export interface ConfidentialityResult {
  readonly pass:   boolean;   // true = safe to share
