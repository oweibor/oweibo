...
exports.BotInstanceManager = void 0;
/**
...
class BotInstanceManager {
    ...
    instances = new Map();
    constructor(adapters, credVault, router) {
        this.adapters = adapters;
        this.credVault = credVault;
        this.router = router;
    }
    async register(reg) {
        ...
        if (!adapter)
            throw new Error(`No adapter registered for platform: ${reg.platform}`);
    ...
    async shutdown() {
        ...
        await Promise.allSettled(entries.map(async ([key, { token, adapter }]) => {
            const colonIdx = key.indexOf(':');
            const tenantId = key.slice(0, colonIdx);
            const platform = key.slice(colonIdx + 1);
            await adapter.stop(token).catch(e => console.error(`[BotInstanceManager] shutdown error for ${key}:`, e));
            await this.credVault.evict(tenantId, platform);
        }));
        this.instances.clear();
    }
}
exports.BotInstanceManager = BotInstanceManager;
//# sourceMappingURL=BotInstanceManager.js.map