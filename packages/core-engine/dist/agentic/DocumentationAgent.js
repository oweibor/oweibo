...
// ── Prompt builders ───────────────────────────────────────────────────────────
function buildUserGuidePrompt(knowledge, clarificationHistory) {
    const k = knowledge;
    ...
    return `
MODULE: ${k['moduleName'] ?? 'Generated Application'}
VERSION: ${k['version'] ?? '1.0.0'}
DESCRIPTION: ${k['domainDescription'] ?? ''}
...
function buildDeveloperPrompt(knowledge, adrText, testSummaries) {
    const k = knowledge;
    ...
    return `
MODULE: ${k['moduleName'] ?? 'Generated Module'}
VERSION: ${k['version'] ?? '1.0.0'}
...
function buildApiReferencePrompt(knowledge) {
    const k = knowledge;
...
MODULE: ${k['moduleName'] ?? 'Generated Module'}
VERSION: ${k['version'] ?? '1.0.0'}
GENERATED AT: ${k['generatedAt'] ?? new Date().toISOString()}
