...
class MemoryOrchestrator {
    ...
    working = (scope) => this.deps.working.for(scope);
    // ── assembleContext ──────────────────────────────────────────────────────
    async assembleContext(input) {
        const { scope, query, kinds, topK = DEFAULT_TOP_K, tokenBudget = DEFAULT_TOKEN_BUDGET, } = input;
        if (!scope.tenantId)
            throw new Error('assembleContext: scope.tenantId is required');
    ...
    // ── recordTurn ───────────────────────────────────────────────────────────
    async recordTurn(scope, turn) {
        if (!scope.sessionId)
            throw new Error('recordTurn: scope.sessionId is required');
        ...
        if (droppedTurns.length && this.deps.summarizer) {
            ...
            await this.deps.shortTerm.setRollingSummary(scope.tenantId, scope.sessionId, newSummary, totalTurns);
        }
    }
    // ── record ───────────────────────────────────────────────────────────────
    async record(input) {
        if (!input.scope.tenantId)
            throw new Error('record: scope.tenantId is required');
        ...
        if (input.kind === 'project-invariant' && input.scope.projectId) {
            ...
            await this.deps.projects.setInvariant(input.scope.tenantId, input.scope.projectId, key, input.body ?? input.summary);
        ...
        if (input.kind === 'conversation-summary' && input.scope.sessionId) {
            ...
            await this.deps.shortTerm.setRollingSummary(input.scope.tenantId, input.scope.sessionId, input.body ?? input.summary, totalTurns);
            return synthesizeEntry(input);
        }
        if (this.deps.semantic)
            return this.deps.semantic.store(input);
        return synthesizeEntry(input);
    }
    // ── consolidate ──────────────────────────────────────────────────────────
    async consolidate(scope, outcome) {
        const entries = [];
    ...
    // ── helpers ──────────────────────────────────────────────────────────────
    async recall(scope, query, kinds, topK) {
        if (!this.deps.semantic)
            return [];
    ...
    formatPromptBlock(project, session, ranked, tokenBudget) {
        const sections = [];
        if (project) {
...
function pickString(detail, key) {
    const v = detail?.[key];
    return typeof v === 'string' ? v : undefined;
}
function pickNumber(detail, key) {
    const v = detail?.[key];
...
function enforceBudget(s, tokenBudget) {
