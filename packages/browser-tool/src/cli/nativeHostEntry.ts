...
import { NativeMessagingHost, type InboundRequest } from '../session/NativeMessagingHost.js';
...
async function connectDaemon(path: string): Promise<Socket> {
...
async function main(): Promise<void> {
  ...
  const host = new NativeMessagingHost({ hmacToken: token });

  // ── Daemon link ────────────────────────────────────────────────────────────
  let daemon: Socket | null = null;
  try { daemon = await connectDaemon(daemonPath); }
  catch (e) {
    logErr(`daemon connect failed (${daemonPath}): ${(e as Error).message}`);
  ...
  daemon.on('data', (chunk: Buffer) => {
    ...
    while (dbuf.length >= 4) {
      ...
      if (dbuf.length < 4 + len) break;
      const body = dbuf.subarray(4, 4 + len).toString('utf8');
      dbuf = dbuf.subarray(4 + len);
      try {
        const msg = JSON.parse(body) as
          | { kind: 'action'; tabId: number; action: unknown; callId: string }
          | { kind: 'gate';   tabId: number; gate: { gateId: string; type: 'dialog' | 'vision-loop'; message: string }; callId: string }
          | { kind: 'response'; callId: string; result?: unknown; error?: string };
        ...
        if (msg.kind === 'action') {
          ...
          host.sendAction(msg.action, msg.tabId)
            ...
            .catch((e: Error) => sendDaemon({ kind: 'response', callId: msg.callId, error: e.message }));
        ...
        logErr(`bad daemon frame: ${(e as Error).message}`);
      }
    }
  });
  ...
  function sendDaemon(obj: unknown): void {
  ...
  host.on('inbound-request', (req: InboundRequest) => {
...
main().catch((e: Error) => { logErr(`fatal: ${e.message}`); process.exit(1); });
