...
import type { SlotPinDetail, SlotPin, CanonicalRole } from '@oweibo/core-contracts';
...
export class CohortRouter {
  private readonly cache = new LRUCache<string, RoleResolution>(500, 60_000);

  constructor(
    private readonly registry:       PromptRegistry,
    private readonly assembler:      PromptAssembler,
    private readonly forceChannel?:  string,
    private readonly bandit?:        BanditService,
    /** Optional Redis subscribe callback for cache invalidation (≤60s lag). */
    private readonly onInvalidate?:  (handler: (msg: string) => void) => void,
    ...
    this.onInvalidate?.((msg) => {
      try {
        const { role, slotId } = JSON.parse(msg) as { role?: string; slotId?: string };
        if (role && slotId) {
  ...
  async resolveForRole(
    ...
    if (this.operationalMode) {
      ...
      if (mode === 0) {
        return {
          promptText:    STABLE_V0_FALLBACKS[role] ?? '',
    ...
    try {
      ...
      if (this.bandit && effectiveChannel !== 'stable-v0') {
        ...
        if (firstSlot) {
          const seed = parseInt(
            createHash('sha256').update(`${taskId}:${firstSlot}`).digest('hex').slice(0, 8), 16,
          );
      ...
      const { text, hash, slotHashes } = await this.assembler.assembleForChannel(
        role, resolvedChannel, 'stable-v0',
      );

      const slotPins: SlotPin[] = Object.entries(slotHashes).map(([slotId, promptHash]) => ({
        slotId,
        promptHash,
  ...
  async resolveAllRoles(
    taskId:  string,
    channel = 'stable-v0',
  ): Promise<Record<CanonicalRole, RoleResolution>> {
    const entries = await Promise.all(
      CANONICAL_ROLES.map(async (role) => [role, await this.resolveForRole(role, taskId, channel)] as const),
    );
    return Object.fromEntries(entries) as Record<CanonicalRole, RoleResolution>;
  }
}

/** Slot IDs for each canonical role (§7.3 slot inventory). */
const SLOTS_FOR_ROLE: Record<CanonicalRole, string[]> = {
...
export const STABLE_V0_FALLBACKS: Record<CanonicalRole, string> = {
