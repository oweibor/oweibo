 ...
 */
export class ModuleReferenceDocTemplate implements IDocTemplate {
  readonly category = 'module-reference' as const;
  ...
  async render(k: CodebaseKnowledge, _ctx: DocTemplateContext, signal?: AbortSignal): Promise<RenderedDocument> {
    ...
    const sections = k.modules.map((mod, idx) => {
      ...
      if (mod.entryPoints.length > 0) {
        ...
        for (const ep of mod.entryPoints) lines.push(`- \`${ep}\``);
        lines.push('');
      }
